...
function App() {
  ...
  // NEW: Fetch Trending Content when User logs in
  useEffect(() => {
  ...
  const logout = () => {
    ...
    setSearchResults([]); // Clear results on logout
  };
  ...
  // NEW: Function to load default trending songs
  const fetchTrending = async () => {
    setLoading(true);
    try {
      // You can change "Trending" to "Top 50", "Hindi", "English", etc.
    ...
    } catch (error) { console.error("Trending fetch failed", error); } 
  ...
  const handleSearch = async () => {
    if (!searchQuery) {
        fetchTrending(); // Reset to trending if search cleared
        return;
    }
  ...
  return (
    <div className="app">
      ...
      <main>
        ...
        {view === 'home' && (
          <>
            {/* Dynamic Title based on whether user is searching or viewing default trending */}
